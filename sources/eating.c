...
static int	get_sleep_time(t_philo *philo,  int old_time)
{
	...
	time = philo->args->time_to_eat + philo->args->time_to_sleep + old_time - cur_time(philo->args->start_time) - 40;
	if (time < 0)
		time = 0;
	return (time);
}
...
static int	get_eat_time(t_philo *philo, int old_time)
{
	...
	(void)old_time;
	time = philo->last_meal - cur_time(philo->args->start_time);
	if (time < 0)
		time = 0;
	return (time);
}
